...
IF OBJECT_ID('[dbo].[fn_SplitInteger]') IS NOT NULL
    DROP FUNCTION [dbo].[fn_SplitInteger]
GO
...
6) Uses a sliding search window, not saving the remaining text on each iteration.  However, 
   saving each item in a temp variable (@tempItemBuffer) was faster than using a CTE to temporarily 
...
02/02/2015  Brad Joss (bradjoss@hotmail.com)            Default to SQL 2012 version of TRY_CONVERT()
08/25/2015  Brad Joss (bradjoss@hotmail.com)            TRY_CONVERT cast [item] to varchar(8000) to avoid truncation error.
10/22/2015  Marco Teixeira (marcoaoteixeira@gmail.com)  Refactor and TODO
---------------------------------------------------------------------------------------------------*/
RETURNS @returnTable TABLE (
    ...
    [item_int] AS (
        ...
        TRY_CONVERT([INT], NULLIF(CAST([item] AS VARCHAR(8000)), '')) -- SQL 2012 Format, faster and safer, but pre-2012 code provided as well...
        /*
...
BEGIN 
    ...
    DECLARE @currentIndex   BIGINT,
        ...
        @delimiterLength    INT,
        @inputTextLength    BIGINT,
        @tempItemBuffer     NVARCHAR(MAX) -- I tried w/o using CTEs, but this way was faster
    ...
    BEGIN -- CHARINDEX MODE (Much faster than PATINDEX mode)
        SELECT  @currentIndex = CHARINDEX(@delimiter, @text), -- TODO: If you want to implment Case Insensitivity here, wrap both in LOWER()
                @previousIndex = 0
        ...
        BEGIN
            ...
            BEGIN
                SELECT
                    ...
                    @tempItemBuffer = LTRIM(RTRIM(SUBSTRING(@text, @previousIndex, (@currentIndex - @previousIndex)))),
                    -- Move previous pointer to end of last found delimiter
                    @previousIndex = @currentIndex + @delimiterLength,
                    -- And update the values for next pass though the loop, finding the next match
                    @currentIndex = CHARINDEX(@delimiter, @text, (@currentIndex + @delimiterLength)) -- TODO: If you want to implment Case Insensitivity here, wrap both in LOWER()

                IF @tempItemBuffer <> '' AND @tempItemBuffer IS NOT NULL -- Only save non empty values
                    INSERT INTO @returnTable(item) VALUES (@tempItemBuffer)
            END

            -- Get the trailing text
            SET @tempItemBuffer = LTRIM(RTRIM(SUBSTRING(@text,@previousIndex,@inputTextLength)))
            IF @tempItemBuffer <> '' AND @tempItemBuffer IS NOT NULL  -- Only save non empty values
                INSERT INTO @returnTable(item) VALUES (@tempItemBuffer)
        END
        ELSE
        BEGIN -- No delimiter found, just return the passed value, trimmed
            INSERT INTO @returnTable(item) 
                SELECT LTRIM(RTRIM(@text)) -- We know this is not null because of the first GIGO check above
        END --  @currentIndex = 0
    END
    ELSE -- @delimiterLength = 1
    BEGIN -- PATINDEX Mode (SLOW!) Try to pass in text that uses single char delimeters when possible
        DECLARE @delimiterPatIndex nvarchar(3002)  -- Assume 3x @delimiter + 2, for escaping every character plus wrapper %

        -- Escape characters that will mess up the like clause, and wrap in wild cards %
        SELECT @delimiterPatIndex = '%' + REPLACE(REPLACE(REPLACE(@delimiter
                , '[','[[]')
                , '%','[%]')
                , '_', '[_]')
            + '%'

        SELECT  @currentIndex = PATINDEX(@delimiterPatIndex, @text), -- TODO: If you want to implment Case Insensitivity here, wrap both in LOWER()
                @previousIndex = 0
        ...
        BEGIN
            ...
            BEGIN
                SELECT 
                    ...
                    , @currentIndex = PATINDEX(@delimiterPatIndex, SUBSTRING(@text, (@currentIndex + @delimiterLength), @inputTextLength)) -- TODO: If you want to implment Case Insensitivity here, wrap both in LOWER()

                IF @tempItemBuffer <> '' AND @tempItemBuffer IS NOT NULL  -- Only save non empty values
                    INSERT INTO @returnTable(item) VALUES (@tempItemBuffer)

                IF @currentIndex > 0 SET @currentIndex = ((@currentIndex + @previousIndex) - 1) -- -1 since PatIndex is 1 based and Substring is 0 based
            END
...
*/
CREATE FUNCTION [dbo].[fn_SplitInteger] (
    @text nvarchar(max)  -- Text to split
)
RETURNS TABLE
AS
...
GRANT REFERENCES, SELECT ON [dbo].[fn_SplitInteger] TO [public] AS [dbo]
GO

-- Precompile, so performance numbers below are not skewed.
SELECT * INTO #Garbage1 FROM [dbo].[fn_Split]('1,2,3',',')
SELECT * INTO #Garbage2 FROM [dbo].[fn_SplitInteger]('1,2,3')
...
INSERT INTO @RESULTS ([msg],[runtime],[expected_count],[actual_count],[expected_sum_len],[actual_sum_len],[delim],[txt])
        ...
        FROM [dbo].[fn_SplitInteger](@int_test)
